...
if [ -f /hdd/hdd-check ] ; then  
	...
	echo -n $WHITE
	/usr/lib/enigma2/python/Plugins/Extensions/BackupSuite-HDD/backupsuite.sh /hdd
	rm -f /hdd/hdd-check
	sync
else
	...
	if  [ $MEDIA = "0" ] ; then
		...
		echo -n $WHITE
		/usr/lib/enigma2/python/Plugins/Extensions/BackupSuite-HDD/backupsuite.sh $MEDIA 
		echo "$HDD_MOUNT" > /tmp/BackupSuite-HDD.log
		sync
	fi
fi
